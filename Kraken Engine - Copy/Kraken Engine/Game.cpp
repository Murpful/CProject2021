	...
	while (eventHandles->size() > 0) {
		...
		if (paused) {
			if (action == "optionScreen") {
				...
				//allObjects.addObject(new DetailObject("nemesis", "assets/nemesis.png", 0, 0, 74, 41, 740, 410));	//-Tony 5/21
				...
				//creatures.push_back(Creature(&mapTiles, &allObjects, "nemesis", 13, { Component(armor,{10}),Component(health,{2}),Component(moveStats,{1,5,2},toward) }));	//-Tony 5/21
				...
				if (playerCharacter.pick.size() == 0) {
					...
					for (int counter = 0; counter < playerCharacter.hand.at(eventHandles->at(0).integers.at(0)).cardEvents.size(); counter++) {
						...
						if (playerCharacter.hand.at(eventHandles->at(0).integers.at(0)).cardEvents.at(counter).action == attack) {
							...
							Attack createdAttack = Attack(thisCardEvent.data.at(0), thisCardEvent.data.at(1), thisCardEvent.data.at(2), thisCardEvent.data.at(3), thisCardEvent.data.at(4), thisCardEvent.actionAnimation);
